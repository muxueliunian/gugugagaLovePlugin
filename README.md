...
- 按日本时间日期生成 `gugugagaMagicCarrot.json` 文件（每日凌晨5点切换）
...
- **时区处理**: 自动使用 "Tokyo Standard Time"，每日凌晨5点（日本时间）切换日期
...
- 数据格式：调整 JSON 序列化逻辑  
- 时区设置：修改 `TzJapan` 和 `GetJapanDateKey()` 方法

## 版本历史

### v1.3.0 (2026-01-02)
- 🌏 时区调整：从东八区（中国时间）改为日本时间
- ⏰ 每日切换时间调整为日本时间凌晨5点（中国时间凌晨4点）
- 🔧 新增 `GetJapanDateKey()` 方法处理日期切换逻辑
